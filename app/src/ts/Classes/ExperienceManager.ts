...
export default class ExperienceManager {
  ...
  private setupSocketListeners(): void {
    ...
    ExperienceSocket.on<ISocketUserData>(
      SocketEvent.USER_DISCONNECT,
      (data) => {
    ...
    ExperienceSocket.on<ISocketClientSpawnPlayerData>(
      SocketEvent.CLIENT_SPAWN_PLAYER,
      (data) => {
        ...
        targetScene.addVisitor(
          ...
          ), // Convert websocket data to Vector3
          new Quaternion(...data.spawnRotation), // Convert websocket data to Quaternion
        );
      },
    );

    ExperienceSocket.on<ISocketClientUpdatePlayerData>(
      SocketEvent.CLIENT_UPDATE_PLAYER,
      (data) => {
        const targetScene = this.scenes.get(data.sceneKey) ?? null;
        ...
        if (
          targetScene &&
          targetScene.visitorAvatars &&
          targetScene.sceneKey === data.sceneKey
        ) {
          ...
          targetScene.visitorAvatars[data.visitorId]?.controls?.update(
            data.delta,
            data.keysPressed,
          );
        }
    ...
    ExperienceSocket.on<ISocketJoinSceneData>(
      SocketEvent.JOIN_SCENE,
      (data) => {
        ...
        const otherScenes: Array<IExperienceScene> = [
          ...this.scenes.values(),
        ].filter((scene) => scene.sceneKey !== data.sceneKey);
        if (otherScenes && otherScenes.length !== 0) {
          const otherScenesWhereVisitorIsStillPresent = otherScenes.filter(
            (scene) => scene.visitorAvatars[data.userId],
          );
    ...
    ExperienceSocket.on<ISocketMessageData>(
  ...
  setActiveScene(key: SceneKey): void {
      ...
      gsap.to(this.activeScene.currentPlayerAvatar.model.scale, {
        ...
        onComplete: () => {
          ...
          ExperienceSocket.emit(SocketEvent.JOIN_SCENE, {
            ...
            sceneKey: key,
            spawnPosition:
              this.activeScene?.currentPlayerAvatar?.model?.position ??
              new Vector3(),
            spawnRotation:
              this.activeScene?.currentPlayerAvatar?.model?.quaternion ??
              new Quaternion(),
          });
        },
      });
    ...
    ExperienceSocket.emit(SocketEvent.JOIN_SCENE, {
      ...
      sceneKey: key,
      spawnPosition:
        this.activeScene?.currentPlayerAvatar?.model?.position ?? new Vector3(),
      spawnRotation:
        this.activeScene?.currentPlayerAvatar?.model?.quaternion ??
        new Quaternion(),
    });
  ...
  private removeEventListeners() {
    ...
    if (this.canvas) {
      this.canvas.removeEventListener('pointermove', (event: PointerEvent) =>
        this.onPointerMove(event),
      );
  ...
  handleRaycaster() {
    ...
    const intersects = this.raycaster.intersectObjects(
      this.activeScene.scene.children,
      true,
    );
  ...
  public fetchOrLoadAvatarCacheEntry(
    ...
    return new Promise(async (resolve, reject) => {
      ...
      try {
        ...
        const gltf = await ThreeLoaders.loadGLTF(
          `/assets/models/avatars/${selectedAvatarId}/avatar.gltf`,
        );
