...
export default class Avatar implements IAvatar {
  ...
  async load() {
    try {
      const { model, animations } =
        await ExperienceManager.instance.fetchOrLoadAvatarCacheEntry(
      ...
      const filteredAnimations = [
        ...animations.filter(
          (animation: AnimationClip) => animation.name !== 'TPose',
        ),
      ];
      ...
      filteredAnimations.forEach((animation: AnimationClip) => {
        this.animationsMap.set(
          animation.name as AnimationName,
