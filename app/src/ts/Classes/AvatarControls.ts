  ...
  update(delta: number, keysPressed: { [key in KeyboardKey]: boolean }) {
    ...
    if (this.currentAction !== play && !this.isJumping && this.avatar) {
      ...
      const animationToPlay =
        this.avatar.animationsMap.get(play) ??
        this.avatar.animationsMap.get(AnimationName.IDLE);
      const currentAnimation = this.avatar.animationsMap.get(
        this.currentAction,
      );
    ...
    if (
      ...
      this.avatar.model?.quaternion.slerp(
      ...
      if (this.avatar.type !== AvatarType.VISITOR) {
        ...
        const visitorWorldDirection = this.avatar.camera.getWorldDirection(
          new Vector3(),
        );
        this.walkDirection = new Vector3(
          visitorWorldDirection.x,
          0,
          visitorWorldDirection.z,
        );
      }
      ...
      if (this.avatar.type === AvatarType.CURRENT_PLAYER) {
        ...
        this.avatar.experienceScene.cameraParent.position.x =
          this.avatar.model.position.x;
        this.avatar.experienceScene.cameraParent.position.z =
          this.avatar.model.position.z;
      }
    }
  }
  ...
  directionOffset(keysPressed: { [key in KeyboardKey]: boolean }) {
    ...
    if (keysPressed[KeyboardKey.KeyW] || keysPressed[KeyboardKey.ArrowUp]) {
      if (keysPressed[KeyboardKey.KeyA] || keysPressed[KeyboardKey.ArrowLeft]) {
        ...
        keysPressed[KeyboardKey.KeyD] ||
        keysPressed[KeyboardKey.ArrowRight]
      ) {
        // z+d
        directionOffset = -Math.PI / 4;
      }
    } else if (
      keysPressed[KeyboardKey.KeyS] ||
      keysPressed[KeyboardKey.ArrowDown]
    ) {
      if (keysPressed[KeyboardKey.KeyA] || keysPressed[KeyboardKey.ArrowLeft]) {
        // s+q
      ...
      keysPressed[KeyboardKey.KeyA] ||
      keysPressed[KeyboardKey.ArrowLeft]
    ) {
      // q
