...
export default abstract class ExperienceScene implements IExperienceScene {
  ...
  public addCurrentPlayer(username: string, selectedAvatarId: number) {
    ...
    this.currentPlayerAvatar = new Avatar(
      username,
      selectedAvatarId,
      this,
      this.camera,
      AvatarType.CURRENT_PLAYER,
    );
  }
  ...
  public update(delta: number): void {
    if (
      this.currentPlayerAvatar &&
      this.currentPlayerAvatar.controls &&
      this.currentPlayerAvatar.model
    ) {
      // Update avatar
      ...
      ExperienceSocket.emit(SocketEvent.CLIENT_UPDATE_PLAYER, {
        ...
        delta: delta,
        keysPressed: ExperienceManager.instance.isInteractive
          ? this.currentPlayerAvatar.controls.keysPressed
          : {},
