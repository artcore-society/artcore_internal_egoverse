...
// Set variables
const chats: Ref<
  Record<string, Array<{ message: string; avatarType: string }>>
> = ref({});
...
const areaButtons: Ref<Record<SceneKey, HTMLElement>> = ref(
  {} as Record<SceneKey, HTMLElement>,
);
...
function submitMessage() {
  ...
  if (ExperienceManager.instance.selectedAvatar.value) {
    ...
    visitorId =
      Object.keys(ExperienceManager.instance.activeScene.visitorAvatars).find(
        (key) => {
          return (
            ExperienceManager.instance.activeScene?.visitorAvatars[key]?.model
              ?.uuid ===
...
watch(ExperienceManager.instance.selectedAvatar, (value) => {
  if (value) {
    // Get visitor id to send message to via websockets by retrieving it from visitors list/object (keys are the visitor id's)
    const visitorId = Object.keys(
      ExperienceManager.instance.activeScene.visitorAvatars,
    ).find((key) => {
      return (
        ExperienceManager.instance.activeScene.visitorAvatars[key].model
...
watch(
  ExperienceManager.instance.incomingVisitorMessageData,
...
onMounted(() => {
  if (canvas.value) {
    ...
    ExperienceManager.instance.addScene(
      SceneKey.CHAT_ROOM,
      new ChatRoomScene(canvas.value, SceneKey.CHAT_ROOM),
    );
...
<template>
  <div class="fixed inset-0">
    ...
    <div
      class="absolute top-5 left-5 z-10 flex justify-center items-center gap-2"
    >
      <Loader v-if="!isReady" />
      ...
      <PrimaryButton
        :ref="
          (el) =>
            (areaButtons[SceneKey.LANDING_AREA] = (
      ...
      <PrimaryButton
        :ref="
          (el) =>
            (areaButtons[SceneKey.MEETING_ROOM] = (
              el as ComponentPublicInstance
            ).$el as HTMLElement)
        "
      ...
      <PrimaryButton
        :ref="
          (el) =>
            (areaButtons[SceneKey.CHAT_ROOM] = (el as ComponentPublicInstance)
              .$el as HTMLElement)
        "
    ...
    <Transition name="fade" mode="out-in" appear>
      <div
        ...
        class="absolute bottom-2 left-2 z-10 flex flex-col gap-4 bg-white p-2 rounded-md"
      >
        <div
          v-for="(chat, visitorId) in chats"
          :key="visitorId"
          class="flex flex-col gap-2 bg-cyan-400 p-2 text-white min-w-42"
        >
          <div
            v-if="
              ExperienceManager.instance.activeScene &&
              ExperienceManager.instance.activeScene.visitorAvatars[visitorId]
            "
          ...
          <div
            ...
            :class="{
              ...
              'bg-green-400 text-white':
                chat.avatarType === AvatarType.CURRENT_PLAYER,
            }"
      ...
      <div
        v-else
        class="absolute bottom-2 left-2 z-10 flex flex-col gap-4 bg-cyan-400 text-white font-bold p-2 rounded-md"
      >
        No chats...
      </div>
    </Transition>
              ...
              <div
                ...
                :class="{
                  'bg-gray-500 text-white text-left':
                    chat.avatarType === AvatarType.VISITOR,
                  'bg-amber-400 text-white text-right':
                    chat.avatarType === AvatarType.CURRENT_PLAYER,
                }"
